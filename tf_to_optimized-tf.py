...
with tf.compat.v1.Session() as sess:
    ...
    transforms = ['remove_nodes(op=Identity)', 'merge_duplicate_nodes', 'strip_unused_nodes',
                  'fold_constants(ignore_errors=true)', 'sort_by_execution_order', 'fold_batch_norms']
    # transforms = ['remove_nodes(op=Identity)', 'merge_duplicate_nodes', 'strip_unused_nodes','fold_constants(ignore_errors=true)', 'fold_batch_norms']

    optimized_graph_def = TransformGraph(graph_def, ['input'], ['output'], transforms)
    tf.io.write_graph(optimized_graph_def, "", 'model_optim.pb', as_text=False) 


    tf.compat.v1.graph_util.import_graph_def(optimized_graph_def, name='')
